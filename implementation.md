...
| 6 | Admin auth + dashboard (metrics, filters, CSV) | ✅ | 1fc5300 | **Cookie SSR** via `@supabase/ssr@^0.12` (version-verified) — Next 16 **`proxy.ts`** (middleware renamed) refreshes the session; `lib/supabase/{server,browser}-ssr.ts`; `lib/admin-auth.ts` `requireAdmin` (cookie + server-side Bearer fallback for tests) → 401/403 via `is_admin()`; `/admin` server-gated, `/admin/login` client. Routes `/api/admin/{orders,metrics,export}` read via the **user-scoped client (RLS-enforced)**. Pure **`lib/metrics.ts`** (revenue Σ, top pizza, busiest-hour IST +5:30) + **`lib/csv.ts`** (RFC-4180 + §17 injection guard). Dashboard: MetricCards/OrdersTable/FilterBar/ExportButton. Metrics accept optional `payment` (superset, for tile coherence). Playwright→P8. 86 unit tests; live smoke: no-session → 401 + `/admin`→307→login. **⏳ Live 403/200 + login smoke await ADMIN_EMAIL/ADMIN_PASSWORD in `.env.local` → `npm run test:admin`.** |
| 7 | AI Feature C — Demand forecasting (★ bonus) | ✅ | 0ada42e | Python `forecast-service/` (FastAPI + scikit-learn **RF + LinearRegression baseline**, IST zero-fill grid, lag_1d/lag_7d, **leak-free temporal split**, RMSE for both). **Deviations (locked w/ user):** seeder is **TS** `web/scripts/seed-orders.ts` (3321 synthetic orders / 75d, phone prefix **9990**, orders-only) not the PRD's Python `seed_orders.py`; chart is **dependency-free inline-SVG** `ForecastChart` (no Recharts, React-19 safe). Synthetic orders **EXCLUDED** from admin metrics/orders/export (phone-prefix `.not(...like '9990%')`) and from recommend popularity (no synthetic line items → clean by construction). Routes `/api/admin/forecast` (mirrors metrics) + `/api/cron/forecast` (constant-time secret → service `/train`). CI gains a Python `pytest` job. **Local-first: Render deploy + live Vercel Cron scheduling → P8.** Live E2E verified: seed → uvicorn `/train` (**rf-v1, 84 rows, RMSE≈2.1**) → Next cron route 200; suites green — 103 unit + **pytest 9** + opt-in `test:forecast 3` / `test:admin 6`. Note: on the smooth synthetic curve RF RMSE (2.09) ≈ Linear (2.00); stored RF as the specified model, both reported (honest Q&A point). |
| 8 | Harden, document & demo-ready | ✅ | bc24f67 | **Docs/hardening/config only — live deploy deferred to developer (local-first).** Root `README.md` (architecture diagram, setup+deploy, Feature A verbatim §23.3 prompt + model rationale, Feature C rf-v1/RMSE, full per-endpoint API §11.4, rubric map); `docs/SECURITY.md` (§17 checklist audited with file evidence; 2 moderate postcss advisories accepted — fix = breaking next downgrade); `docs/DEMO.md` (§21 Q&A, live `DISCOUNT_THRESHOLD` 5→3). Config: `web/vercel.json` (daily cron 02:00 UTC), `render.yaml` (Render Blueprint, `$PORT`/`0.0.0.0`). **Hardening:** cron route now accepts Vercel's `Authorization: Bearer <CRON_SECRET>` (+ `x-cron-secret` fallback) — the Phase-7 route would 401 every Vercel cron run; CI adds `next build` (placeholder-env verified); `layout.tsx` metadataBase. **Playwright skipped** (covered by RTL happy-path + opt-in integration). 104 web unit + 9 pytest + tsc + lint + build + `npm audit` green; live OpenRouter re-check still **429** (deterministic fallback covers it). **USER-OWNED (deferred):** Vercel + Render deploy, Vercel Cron activation, share read-only Supabase, provision grader admin (`npm run provision:admin` ready), Loom, live presentation, multi-member commit history. |
...
**Definition of Done:**
- [x] Returning phone → tailored pick; new phone → deterministic pick framed as popular (cold start). *(Route branches on history; cold-start live smoke → 200 with a house-favourite pick; opt-in `test:recommend` 3/3 (bad phone 400, cold-start, returning — all on-menu).* **⚠ Caveat:** a *successful live LLM* pick could not be observed this session — OpenRouter's free tier 429-throttles every call, so the deterministic fallback served (by design). LLM path proven by unit tests (rotation/defensive-parse/menu-validation) + the route returning a valid on-menu pick. **Re-verify a live LLM recommendation before the Phase 8 demo, when the free tier isn't throttled.**)*
...
> **Phase-6 decisions (locked with the user):** (a) **cookie SSR via `@supabase/ssr`** (not token-based) — Next 16 `proxy.ts` refreshes the session, `/admin` gated server-side; (b) admin creds — **user adds `ADMIN_EMAIL`/`ADMIN_PASSWORD` to `.env.local`** for the live login + `test:admin`; (c) **Playwright deferred to Phase 8**. Also: `requireAdmin` accepts a server-side `Authorization: Bearer` fallback so the opt-in integration tests exercise the real gate headlessly (browser always uses cookies); `/api/admin/metrics` accepts an optional `payment` param (documented superset of §11.3) so tiles match the active filter.

**Definition of Done:**
- [x] Admin login works; non-admin gets 403; missing session gets 401. *(No-session → **401** live-verified on `/api/admin/*`; `/admin` → **307 → /admin/login**. 403 non-admin + 200 admin + the login smoke are covered by opt-in `test:admin` — **run after adding the creds to `.env.local`**.)*
- [x] Filters, revenue, top pizza, busiest-hour (IST) all correct and respect filters. *(Pure `lib/metrics.ts` unit-tested incl. an IST hour crossing midnight; routes apply `from`/`to`/`payment`; metrics honor `payment` too.)*
- [x] CSV downloads; cells starting with `= + - @` (and tab/CR) are prefixed with `'`. *(`csv.test.ts` covers the guard + RFC-4180 quoting incl. the CR guard-inside-quotes case; export route sets `text/csv` + `attachment`.)*
- [x] **Tests:** CSV guard + metric computations (unit); MetricCards/FilterBar (component); opt-in `tests/admin.integration.test.ts` (401/403/200, pagination, metrics shape, CSV headers). *(86 unit passing / 22 opt-in skipped; `tsc`+lint+`next build` green; `proxy.ts` recognized.)* Playwright e2e → Phase 8.
- [x] Commit: `1fc5300` — `feat: admin auth + dashboard (orders, revenue, top pizza, busiest hour, CSV export)`
...
**Definition of Done:**
- [x] `/train` populates `demand_forecasts` (84 rows, one `generated_at`); dashboard renders the chart + top-3 peaks + RMSE caption; cron wired (local). *(`/train` live-verified → rf-v1; `ForecastChart` component-tested + data-backed; a final browser eyeball is recommended, per the Phase 1/6 style.)*
- [x] No Python at request time — Next.js only reads `demand_forecasts` (`/api/admin/forecast` via the RLS-scoped client). *(Python service is triggered offline by `/api/cron/forecast`.)*
- [x] **Tests:** pytest for the feature builder, the temporal split (no leakage), RMSE calc, and the clamped forward forecast (**9 passing**); TS `cron.test.ts` (bad secret → 401) + opt-in `tests/forecast.integration.test.ts` (401/403/200 shape). Plus `synthetic`/`forecast` lib units + `ForecastChart` component tests.
- [x] Commit: `0ada42e` — `feat: demand forecasting service + dashboard (Feature C, bonus)`

> **Phase-7 deviations & decisions (locked with the user, override the docs):** (a) **service scope** — build + run the FastAPI service locally; **defer the Render deploy + live Vercel Cron schedule to Phase 8** (local-first); (b) **seeder in TypeScript** (`web/scripts/seed-orders.ts`, mirrors `seed-menu.ts`) instead of the PRD's `supabase/seed/seed_orders.py`; (c) **dependency-free inline-SVG chart** instead of Recharts (React 19 / Next 16 safe); (d) **synthetic orders excluded** from admin metrics + Feature A via phone prefix `9990` (`lib/synthetic.ts`) — orders-only seed keeps recommend counts clean by construction; (e) **store RandomForest** (`rf-v1`) with LinearRegression reported as a baseline.
...
**Definition of Done:**
- [x] Full test suite green; `npm audit` clean. *(104 web unit + 9 pytest; tsc + lint + `next build` + `npm audit --audit-level=high` all green locally; CI now also runs `next build`. GitHub Actions confirms on push.)*
- [ ] End-to-end smoke on the **live URL**: ordering + both AI features + admin. *(Deferred — the live Vercel/Render deploy is user-owned. Locally verified in Phases 4–7; `docs/DEMO.md` has the demo-day checklist.)*
- [~] §22 submission checklist / §2 rubric map. *(Code + docs complete: README, security audit, Q&A prep, deploy config. Remaining items are user-owned: live URL, read-only Supabase access, grader admin, Loom, live presentation, multi-member commit history.)*
- [x] Commit: `bc24f67` — `chore: security pass, README + system prompts, demo-ready`

> **Phase-8 decisions (locked with the user, override the docs):** (a) **docs/hardening/config only** — the live Vercel + Render deploy is deferred to the developer (local-first); (b) forecast service kept **pre-populated + config-only** (no Render deploy now; `render.yaml` + `vercel.json` ship for a later one-click deploy); (c) **Playwright skipped** — e2e is covered by the RTL happy-path + opt-in integration suites; (d) cron route hardened to Vercel's `Authorization: Bearer` convention (the deploy-critical fix). Live OpenRouter LLM path remains 429-throttled on the free tier (Phase 5 caveat stands; deterministic fallback covers it — re-verify before the demo when unthrottled).
